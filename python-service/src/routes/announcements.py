...
def _serialize_announcement(a, uid=None):
    d = dict(a)
    if d.get('created_at'):
    ...
    if uid is not None:
        d['user_liked'] = bool(d.get('user_liked'))
...
def get_announcements():
    ...
    try:
        ...
        for a in announcements:
            result.append(_serialize_announcement(a, uid=user_id))
...
@announcements_bp.route("/api/announcements/public", methods=["GET", "OPTIONS"])
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO announcements
                (user_id, title, content, image_url, image_urls, target_group_id, target_heads_only, is_public)
        ...
        d = _serialize_announcement(announcement, uid=user_id)
        d.update({
    ...
    try:
        ...
        cursor.execute('''
            ...
            SET title = %s, content = %s, image_url = %s, image_urls = %s,
                target_group_id = %s, target_heads_only = %s, is_public = %s,
