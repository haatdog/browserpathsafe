...
function MultiImageUploader({ images, onChange, max = 5, accentColor = 'purple' }: MultiImageUploaderProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {images.length < max && (
        <label
          className={`flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-lg cursor-pointer transition ${dropBg}`}
        ...
        >
          <div className="flex flex-col items-center gap-1 text-gray-500 pointer-events-none">
            ...
            <span className="text-sm">{images.length === 0 ? 'Upload photos' : 'Add more'}</span>
            <span className="text-xs text-gray-400">{images.length}/{max} • up to 5MB each</span>
          </div>
...
// ── Inline mini slideshow (for detail pane) ───────────────────────────────────
          ...
          {images.map((src, idx) => (
            <button key={idx} onClick={() => setCurrent(idx)}
              className={`w-12 h-12 rounded-md overflow-hidden border-2 transition-all flex-shrink-0 ${
                idx === current ? 'border-purple-500 opacity-100' : 'border-transparent opacity-50 hover:opacity-75'
              }`}>
              <img src={src} alt="" className="w-full h-full object-cover" />
            </button>
          ))}
...
// ── Helper: get images array from evaluation ─────────────────────────────────
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold text-white">Evacuation Evaluations — Executive View</h2>
            <p className="text-purple-100 text-sm mt-1">{event.title}</p>
          </div>
        ...
        <div className="bg-purple-50 border-b border-purple-200 px-6 py-4 flex-shrink-0">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div>
              ...
              <p className="font-semibold text-gray-900 text-sm">{new Date(event.start_time).toLocaleDateString()}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-1">Submissions</p>
              <p className="font-semibold text-gray-900">{stats.totalSubmissions}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-1">Total Participants</p>
              <p className="font-semibold text-purple-700 text-lg">{stats.totalParticipants}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-1">Total Males</p>
              <p className="font-semibold text-blue-600 text-lg">{stats.totalMale}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-1">Total Females</p>
              <p className="font-semibold text-pink-600 text-lg">{stats.totalFemale}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex border-b border-gray-200 flex-shrink-0">
          <button onClick={() => setActiveTab('view')}
            className={`flex items-center gap-2 px-6 py-3 text-sm font-medium transition border-b-2 ${
              activeTab === 'view' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50'
            }`}>
            <FileText className="w-4 h-4" />
            Member Evaluations
            {evaluations.length > 0 && (
              <span className="ml-1 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full">{evaluations.length}</span>
            )}
          </button>
          <button onClick={() => { setActiveTab('submit'); setSubmitSuccess(false); }}
            className={`flex items-center gap-2 px-6 py-3 text-sm font-medium transition border-b-2 ${
              activeTab === 'submit' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50'
            }`}>
                ...
                <div className="p-4">
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600 text-sm">No evaluations submitted yet</p>
                    </div>
                  ) : (
                          ...
                          <button key={ev.id} onClick={() => setSelectedEvaluation(ev)}
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <div className="flex items-center gap-2 text-xs text-gray-500">
                                ...
                                {ev.male_count + ev.female_count} participants
                              </div>
                              <span className="text-xs text-blue-600">{ev.male_count}M / {ev.female_count}F</span>
                            </div>
                            {imgCount > 0 && (
                ...
                {selectedEvaluation ? (
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-4">
                      ...
                      <div className="mb-4">
                        <div className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1">
                          <User className="w-4 h-4 text-purple-600" />Instructor / Representative
                        </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1">
                          <GraduationCap className="w-4 h-4 text-purple-600" />Program / Class
                        </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1">
                          <MapPin className="w-4 h-4 text-purple-600" />Classroom / Office
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-4">
                        <div className="bg-white rounded-lg p-3 border border-gray-200 text-center">
                          <div className="text-xs text-gray-600 mb-1">Male</div>
                          <div className="text-2xl font-bold text-blue-600">{selectedEvaluation.male_count}</div>
                        </div>
                        <div className="bg-white rounded-lg p-3 border border-gray-200 text-center">
                          <div className="text-xs text-gray-600 mb-1">Female</div>
                          <div className="text-2xl font-bold text-pink-600">{selectedEvaluation.female_count}</div>
                        </div>
                        <div className="bg-white rounded-lg p-3 border border-gray-200 text-center">
                          <div className="text-xs text-gray-600 mb-1">Total</div>
                          <div className="text-2xl font-bold text-purple-600">
                            {selectedEvaluation.male_count + selectedEvaluation.female_count}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                          <MessageSquare className="w-4 h-4 text-purple-600" />Comments and Suggestions
                        </div>
                        <div className="bg-white border border-gray-200 rounded-lg p-3">
                          {selectedEvaluation.comments
                            ? <p className="text-gray-700 whitespace-pre-wrap text-sm">{selectedEvaluation.comments}</p>
                            : <p className="text-gray-400 italic text-sm">No comments provided</p>
                          }
                        </div>
                      </div>

                      {/* Photos mini-slideshow */}
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600">Select an evaluation to view details</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="max-w-2xl mx-auto p-6">
                {submitSuccess ? (
                  <div className="flex flex-col items-center justify-center py-16 gap-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">Evaluation Submitted!</h3>
                    <p className="text-gray-600 text-center">
                      Your evaluation for <strong>{event.title}</strong> has been recorded.
                    </p>
                  ...
                  <>
                    <div className="mb-6">
                      <h3 className="text-lg font-bold text-gray-900">Submit Your Evaluation</h3>
                      <p className="text-sm text-gray-600 mt-1">
                        Record your own headcount and observations for <strong>{event.title}</strong>.
                      </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmitEvaluation} className="space-y-5">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Your Name / Representative *</label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Classroom / Office *</label>
                      ...
                      <div className="bg-purple-50 border border-purple-200 rounded-lg px-4 py-3 flex items-center justify-between text-sm">
                        ...
                        <span className="font-bold text-purple-700 text-lg">{form.male_count + form.female_count}</span>
                      </div>
        ...
        <div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            {activeTab === 'view' && (
              <button onClick={() => alert('Report download coming soon!')}
