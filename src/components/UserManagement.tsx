...
import {
  Users, UserPlus, UserMinus, Trash2, Shield, User, Crown, Loader,
  Tag, Star, Plus, X, Pencil, Check
} from 'lucide-react';
...
const getRoleIcon = (role: string) => {
  ...
  if (role === 'executive') return <Shield className="w-4 h-4 text-blue-600"  />;
...
export default function UserManagement({
  ...
  // ── Assign existing member picker (Unit Head only) ─────────────────────────
  ...
  // ── Derived: unassigned members for Unit Head picker ─────────────────────
  const unassignedMembers = users.filter(
    u => u.role === 'member' && !u.group_id
  );
  ...
  const handleAssignMembers = async () => {
    ...
    try {
      await Promise.all(pickerSelected.map(uid =>
        fetch(`${API_BASE}/api/users/${uid}/group`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ group_id: currentUserGroupId, is_head: false }),
        })
      ));
      setPickerSelected([]);
  ...
  // ── Kick member from group (Unit Head or Admin) ────────────────────────────
  const handleKickMember = async (userId: string, userName: string) => {
    if (!confirm(`Remove ${userName} from the group?`)) return;
    try {
      await fetch(`${API_BASE}/api/users/${userId}/group`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ group_id: null, is_head: false }),
      });
      await loadAll();
    } catch (err: any) {
      setError(err.message);
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      // Unit Heads can only add members to their own group
      const payload = {
        email:      newUser.email,
  ...
  // ── Save group / Unit Head assignment ──────────────────────────────────────
  const handleSaveAssignment = async (userId: string) => {
    try {
      await fetch(`${API_BASE}/api/users/${userId}/group`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ group_id: editGroup || null, is_head: editIsHead }),
      });
      setEditingUserId(null);
      await loadAll();
      // If the admin just made themselves (or current user) a head, refresh the page
      // so the sidebar picks up the updated is_head from /api/auth/me
  ...
  // ── Visibility rules ───────────────────────────────────────────────────────
  // Unit Heads only see members of their own group; admins see everyone
  const visibleUsers = isUnitHead && !isAdmin
    ? users.filter(u => u.group_id === currentUserGroupId)
    : users;

  const canRenameGroup = (g: Group) =>
    isAdmin || (isUnitHead && g.id === currentUserGroupId);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
            <Tag className="w-4 h-4 text-blue-600" /> Groups
          </h3>
          {isAdmin && (
            ...
            <div key={g.id}
              ...
              {renamingGroupId === g.id ? (
                ...
                <div className="flex items-center gap-1.5">
                  <input autoFocus value={renameGroupValue}
                    onChange={e => setRenameGroupValue(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === 'Enter')  handleRenameGroup(g.id);
                      if (e.key === 'Escape') setRenamingGroupId(null);
                    }}
                    className="w-44 px-2 py-0.5 text-sm border border-blue-400 rounded bg-white text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                ...
                <>
                  <span>{g.name}</span>
                  {canRenameGroup(g) && (
                    <button onClick={() => startRenaming(g)} title="Rename group"
                  ...
                  {isAdmin && (
                    <button onClick={() => handleDeleteGroup(g.id)} title="Delete group"
                      className="ml-0.5 opacity-60 hover:opacity-100 hover:text-red-600 transition">
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </>
            ...
            <div key={user.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Kick: admin can kick anyone in a group; unit head can kick members of their own group */}
                  {user.group_id !== null && user.role === 'member' && (isAdmin || (isUnitHead && user.group_id === currentUserGroupId)) && (
                    <button
                      onClick={() => handleKickMember(user.id, getDisplayName(user))}
              ...
              {/* Inline assignment edit — admin only */}
              {editingUserId === user.id && (
                <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-3 flex-wrap">
                  <select value={editGroup}
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer select-none">
                    <input type="checkbox" checked={editIsHead} onChange={e => setEditIsHead(e.target.checked)}
          ...
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 style={T.pageTitle}>
                {isUnitHead && !isAdmin ? 'Add Member to Group' : 'Create New User'}
              </h3>
            ...
            {isUnitHead && !isAdmin && (
              <div className="mb-4 bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-2.5">
                <p className="text-sm text-indigo-700">
                  New member will be added to{' '}
                  <span className="font-semibold">
                    {groups.find(g => g.id === currentUserGroupId)?.name ?? 'your group'}
                  </span>.
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
              ...
              {/* Role, group, Unit Head — admin only */}
              {isAdmin && (
                <>
                  <div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowGroupModal(false)} className="text-gray-400 hover:text-gray-600 transition">
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Select from unassigned members
                </p>
              </div>
            ...
            {unassignedMembers.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="mt-1" style={T.meta}>All members are already in a group.</p>
              </div>
            ) : (
            ...
            <div className="flex gap-3 pt-2 border-t border-gray-100">
              ...
              <button onClick={handleAssignMembers}
                disabled={pickerSaving || pickerSelected.length === 0}
                className="flex-1 px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition disabled:opacity-50 flex items-center justify-center gap-2">
                {pickerSaving
                  ? <><Loader className="w-4 h-4 animate-spin" /> Adding…</>
                  : `Add ${pickerSelected.length || ''} Member${pickerSelected.length !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        </div>
